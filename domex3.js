...
Data.forEach((compObj)=>{
  ...
  for(let i in compObj){
    if(i==="img_src") row+=`<td><img width="96px" src=${compObj[i]}`+` alt="loading..." ></td>`;
    else row+=`<td>${compObj[i]}</td>`;
    
  }

  let rowEle=document.createElement('tr');
  rowEle.innerHTML=row;
  document.getElementById("tablecontent").appendChild(rowEle);
